      ...
      <g className="architecture-edges">
        {layout.edges.map((edge) => (
          <path
            ...
            fill="none"
            key={edge.id}
            markerEnd="url(#policy-architecture-arrow)"
          />
        ))}
      </g>
...
function buildArchitectureGraph(preview: PolicyArchitecturePreview): ArchitectureGraph {
  ...
  const sourceNodes = preview.architecture_lanes.flatMap((lane) => lane.nodes);
  ...
  connectLane(preview.architecture_lanes[0]?.nodes ?? [], edges);
  connectLane(preview.architecture_lanes[1]?.nodes ?? [], edges);
  addEdgeFromLastNode(preview.architecture_lanes[0]?.nodes ?? [], "concat:image", edges);
  addEdgeFromLastNode(preview.architecture_lanes[1]?.nodes ?? [], "concat:state", edges);
...
function concatPorts() {
  ...
  return [
    ...
    concatPort("concat:state", 0, diagramMetrics.junctionSize - 5, "WEST"),
    concatPort("concat:out", diagramMetrics.junctionSize - 2, center, "EAST"),
  ];
}

function concatPort(id: string, x: number, y: number, side: "EAST" | "NORTH" | "WEST") {
  return {
    id,
    height: 2,
    layoutOptions: {
...
function toRenderedGraph(elkGraph: ElkNode, visuals: Map<string, NodeVisual>): RenderedGraph {
  ...
  const edges = (elkGraph.edges ?? []).flatMap((edge) => edgePaths(edge));
  const width = Math.max(numberOrZero(elkGraph.width), maxNodeExtent(nodes, "x"));
  const height = Math.max(numberOrZero(elkGraph.height), maxNodeExtent(nodes, "y"));
...
function edgePaths(edge: ElkExtendedEdge): RenderedEdge[] {
  return (edge.sections ?? []).map((section) => ({
    id: `${edge.id}-${section.id}`,
    path: pathForSection(edge.id, section),
  }));
}

function pathForSection(edgeId: string, section: ElkEdgeSection) {
  if (edgeId.endsWith("-concat:state")) {
    return stateConcatPath(section);
  }
...
function stateConcatPath(section: ElkEdgeSection) {
  const start = section.startPoint;
  const end = section.endPoint;
  const midX = Math.max(start.x + 24, (start.x + end.x) / 2);
  return [
