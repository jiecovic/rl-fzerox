...
_REPO_ROOT_RELATIVE_PREFIXES = frozenset({"checkpoints", "local"})
...
def resolve_config_path_value(raw_path: str, *, config_dir: Path) -> Path:
    """Resolve a serialized runtime-config path.

    Saved run manifests resolve most relative paths against their own directory,
    while repo-root prefixes such as ``local/...`` and ``checkpoints/...`` stay
...
def config_path_resolution_base_dir(config_dir: Path) -> Path:
    """Return the base directory used to resolve serialized runtime paths."""
