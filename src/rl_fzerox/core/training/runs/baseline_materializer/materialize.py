...
def _materialize_track_sampling(
    ...
    *,
    ...
    _report_startup(
        startup_reporter,
        "startup_materialize",
        f"Materializing {sampling_label} baselines for {len(config.entries)} entries",
    )
    entries = tuple(
        entry.model_copy(
            update={
...
def _materializer_mode(request: BaselineRequest) -> str:
    ...
    if request.mode == "time_attack":
        return BASELINE_MATERIALIZER_SETTINGS.boot_menu_time_attack_mode
    if request.mode == "gp_race":
        return BASELINE_MATERIALIZER_SETTINGS.boot_menu_gp_race_mode
    raise ValueError(f"Unsupported race-start materializer mode {request.mode!r}")
...
def _generate_race_start_variant_state(
    ...
    *,
    ...
    mode = _validated_request_mode(request.mode)
