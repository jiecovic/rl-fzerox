...
export const trackSamplingRuntimeEntrySchema = z.object({
  ...
  completion_sample_count: z.number().int().nonnegative(),
  completion_fraction_total: z.number().nonnegative(),
...
export const engineTuningRuntimeCandidateSchema = z.object({
  ...
  return_count: z.number().int().nonnegative().default(0),
  ...
  mean_completion_score: z.number().min(0).max(1).nullable().default(null),
  best_completion_score: z.number().min(0).max(1).nullable().default(null),
  finish_rate: z.number().min(0).max(1).nullable().default(null),
  failure_rate: z.number().min(0).max(1).nullable().default(null),
  mean_finish_score: z.number().nullable().default(null),
  mean_return_score: z.number().nullable().default(null),
  best_return_score: z.number().nullable().default(null),
...
export const engineTuningRuntimeCandidateEstimateSchema = z.object({
  ...
  mean_finish_time_ms: z.number().int().positive().nullable().default(null),
  mean_completion_score: z.number().min(0).max(1).nullable().default(null),
  best_completion_score: z.number().min(0).max(1).nullable().default(null),
  finish_rate: z.number().min(0).max(1).nullable().default(null),
  failure_rate: z.number().min(0).max(1).nullable().default(null),
...
export const engineTuningRuntimeContextSchema = z.object({
  ...
  recommended_engine_setting_raw_value: z
    .number()
    .int()
    .min(ENGINE_SLIDER.minStep)
    .max(ENGINE_SLIDER.maxStep),
