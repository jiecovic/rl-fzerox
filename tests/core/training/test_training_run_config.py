...
def test_load_train_run_config_rejects_removed_adaptive_step_balance_fields(
    tmp_path: Path,
) -> None:
    core_path = tmp_path / "mupen64plus_next_libretro.so"
    rom_path = tmp_path / "fzerox.n64"
    ...
    with pytest.raises(ValidationError):
        load_train_run_config(run_paths.run_dir)


@pytest.mark.parametrize("old_mode", ("adaptive_step_balanced", "balanced", "random"))
def test_load_train_run_config_rejects_removed_track_sampling_mode_names(
    tmp_path: Path,
    old_mode: str,
) -> None:
    core_path = tmp_path / "mupen64plus_next_libretro.so"
